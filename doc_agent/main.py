...
def extract_cited_chunks(answer: str) -> list[int]:
    return sorted(
        set(int(x) for x in re.findall(r"\[chunk\s+(\d+)\]", answer, flags=re.IGNORECASE))
    )

def main():
    # Only do setup inside main (no top-level side effects)
    load_dotenv()
    init_db()
        ...
        if uploaded is not None:
            ...
            if st.button("Store document"):
                ...
                if not text or len(text) < 50:
                    st.error(
                        "Could not extract meaningful text. If this PDF is scanned, OCR is not supported in this starter."
                    )
                else:
                    # Store without model
